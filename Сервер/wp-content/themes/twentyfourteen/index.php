		...
		<div class='content'>
			...
			<?php foreach ($posts as $post) : setup_postdata ($post); ?>
				...
				echo $temp;?>">
            		...
            		<h2><?php the_title(); ?></h2>
            		<?php the_excerpt(); ?> 
            		<a class="button" href="<?php the_permalink(); ?>" title="Узнать больше о наших продуктах">Больше</a>
 				</div>
		...
		<div class='content'>
			...
			$tempid = 0;
			 ?>
			  <?php if ($posts) : ?>
				<?php foreach ($posts as $post) : setup_postdata ($post); ?>
 				<div class="foto_galery-item" title="Просмотреть">
 						<?php the_post_thumbnail('full'); ?> 
            		<h3><?php the_title(); ?>
            			<em></em>
            		</h3>
			...
			<a class="button" href="/galery/" title="Просмотреть больше фотографий">Ещё</a>
		</div>
	</div>
	<div class="last_news">
		<div class="content">
			...
			<?php $posts = get_posts ("category=6&orderby=date&numberposts=2"); ?>
			<?php if ($posts) : ?>
			<?php foreach ($posts as $post) : setup_postdata ($post); ?>
 				<div class="item">
 					<div class="time">
